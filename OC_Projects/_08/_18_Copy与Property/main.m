...
int main(int argc, const char * argv[]) {
    ...
    // block 默认 存储在 栈中, 栈中的block访问到了外界的对象, 不会对对象进行retain
    // block 如果在 堆中, 如果在block中访问了外界的对象, 会对外界的对象进行一次retain
    Person *p = [[Person alloc] init];
    ...
    // 3.注意点: copy block 之后引发循环引用    \
        如果对象中的 block 又用到了对象自己,   \
        那么为了避免内存泄露, 应该将对象修饰为__block
