...
-(void)viewWillAppear:(BOOL)animated{
    ...
    //移除系统tabbar的子控件 UITabBarButton
    //UITabBarButton是私有属性
    for (UIView *view in self.tabBar.subviews) {
        
        //逆向思维。判断下当前控件是不是WJZTabBar，如果不是直接移除
        if (![view isKindOfClass:[WJZTabBar class]]) {
...
-(void)setupAllChildViewController{
    ...
    // 3.发现
    //3.1加载 storyboard
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"WJZDiscoverTableViewController" bundle:nil];
    //3.2初始化箭头指向的控制器
    WJZDiscoverTableViewController *discover = [storyboard instantiateInitialViewController];
